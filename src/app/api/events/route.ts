...
export async function POST(request: NextRequest) {
  try {
    ...
    const newEventDocument = {
      ...
      rsvpIds: [], // Initialize with empty ObjectId array for RSVPs
      // _id will be generated by MongoDB
    };

    const eventsCollection = db.collection('events'); // Omit<Event, 'id' | 'organizer' | 'rsvps'>
    ...
    const createdEventForClient: Event = {
        ...
        organizerId: newEventDocument.organizerId.toHexString(),
        ...
        rsvpIds: [], // Start with empty string array for client
...
export async function GET(request: NextRequest) {
  try {
    ...
    const enrichedEvents: Event[] = await Promise.all(
      eventsFromDb.map(async (eventDoc) => {
        const organizerDoc = await usersCollection.findOne({ _id: new ObjectId(eventDoc.organizerId as string) });
        ...
        if (eventDoc.rsvpIds && eventDoc.rsvpIds.length > 0) {
          // Ensure rsvpIds are ObjectIds for the $in query
        ...
        return {
          ...
          organizerId: eventDoc.organizerId.toString(),
          organizer: organizerForClient || { id: 'unknown', name: 'Unknown User', email: '', reputation: 0, joinedDate: new Date().toISOString() } as User,
          rsvpIds: eventDoc.rsvpIds?.map((id: ObjectId | string) => id.toString()) || [],
          rsvps: rsvpsForClient,
          // _id is already an ObjectId from eventDoc
        } as Event;
      })
    );
