...
// Schema for validating POST request body for event creation
const eventFormSchema = z.object({
  ...
  path: ["endTime"], // Field to associate error with
});
...
export async function POST(request: NextRequest) {
  try {
    ...
    const newEventDocument = {
      ...
      rsvpIds: [] as ObjectId[], // Initialize with empty ObjectId array for RSVPs
    ...
    // Prepare event for client response
    const createdEventForClient: Event = {
        ...newEventDocument,
        _id: result.insertedId,
        id: result.insertedId.toHexString(),
        organizerId: newEventDocument.organizerId, // Keep as ObjectId for internal consistency, convert to string on demand
        organizer: { // Embed organizer details
             _id: organizer._id,
        ...
        rsvpIds: [], // Start with empty ObjectId array
        rsvps: [],   // Start with empty User array for client
    };
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (rsvpdByParam && ObjectId.isValid(rsvpdByParam)) {
      query.rsvpIds = new ObjectId(rsvpdByParam); // Check if user's ObjectId is in the rsvpIds array
    }
    ...
    const enrichedEvents: Event[] = await Promise.all(
      eventsFromDb.map(async (eventDoc: any) => { // Use 'any' here as eventDoc is from DB and might not match Event type exactly yet
        ...
        return {
          ...
          organizerId: eventDoc.organizerId, // Keep as ObjectId
          organizer: organizerForClient || { id: 'unknown', name: 'Unknown User', email: '', reputation: 0, joinedDate: new Date().toISOString() } as User,
          rsvpIds: eventDoc.rsvpIds?.map((id: ObjectId | string) => typeof id === 'string' ? new ObjectId(id) : id) || [], // Ensure ObjectIds
          rsvps: rsvpsForClient,
          _id: eventDoc._id, // Ensure _id is ObjectId
        } as Event;
      })
    );
