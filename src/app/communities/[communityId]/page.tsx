...
import { UsersRound, UserPlus, LogOut, Loader2, AlertTriangle, Settings, ShieldCheck, Info, MessageSquare, CalendarDays } from 'lucide-react';
...
export default function CommunityDetailPage() {
  ...
  const handleJoinLeaveCommunity = async () => {
    ...
    try {
        ...
        await fetchCommunityDetails(); // Re-fetch community details to update member count and status
        await refreshUser(); // Refresh user context to update their communityIds list
        ...
        <CardHeader className="p-6 border-b">
            ...
            <div className="flex items-center space-x-6 text-sm text-muted-foreground mt-4">
                <div className="flex items-center">
                    <UsersRound className="h-4 w-4 mr-1.5 text-primary" /> {community.memberCount || 0} Members
                </div>
                {community.creator && (
                     <div className="flex items-center">
              ...
              <TabsContent value="discussion">
                <Card>
                    ...
                    <CardContent className="text-center py-10 text-muted-foreground">
                        ...
                        <p>Community-specific posts will appear here.</p>
                        <p className="text-xs mt-1">(Feature in development)</p>
                        {isMember && <Button size="sm" className="mt-4" onClick={()=>toast.info("Posting to community feed not implemented yet.")}>Create Post in Community</Button>}
                    </CardContent>
                </Card>
              </TabsContent>
              <TabsContent value="events">
                 <Card>
                    <CardHeader><CardTitle>Community Events</CardTitle></CardHeader>
                    <CardContent className="text-center py-10 text-muted-foreground">
                        <CalendarDays className="h-12 w-12 mx-auto mb-4 opacity-30" />
                        <p>Events organized by this community will be listed here.</p>
                        <p className="text-xs mt-1">(Feature in development)</p>
                         {isMember && <Button size="sm" className="mt-4" onClick={()=>toast.info("Creating community event not implemented yet.")}>Create Community Event</Button>}
                    </CardContent>
                </Card>
              </TabsContent>
              <TabsContent value="members">
                 <Card>
                    <CardHeader><CardTitle>Members ({community.memberCount || 0})</CardTitle></CardHeader>
                    <CardContent className="text-center py-10 text-muted-foreground">
                        <UsersRound className="h-12 w-12 mx-auto mb-4 opacity-30" />
                        <p>A list of community members will be displayed here.</p>
                        <p className="text-xs mt-1">(Feature in development)</p>
                    </CardContent>
                </Card>
              </TabsContent>
